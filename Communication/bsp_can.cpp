...
void can_filter_mask_config(CAN_HandleTypeDef *hcan, uint8_t object_para, uint32_t id, uint32_t mask_id)
{
    ...
    if(id_type_flag == CAN_STDID)
    {
        ...
        if (frame_type == CAN_REMOTE_TYPE) 
        {
        filter.FilterIdLow |= 0x0002;                       // 设置RTR位为1
        filter.FilterMaskIdLow |= 0x0002;                   // 掩码对应位也设为1
        }
    }
    else if(id_type_flag == CAN_EXTID)
    {
        // 扩展ID
        filter.FilterIdHigh = (id >> 13) & 0xFFFF;
        filter.FilterIdLow = ((id << 3) & 0xFFF8) | 0x0004;                 // IDE=1
        filter.FilterMaskIdHigh = (mask_id >> 13) & 0xFFFF;
        filter.FilterMaskIdLow = ((mask_id << 3) & 0xFFF8) | 0x0004;        // IDE=1
